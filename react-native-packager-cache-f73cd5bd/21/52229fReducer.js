...
function inject(state, action, props, scenes) {
  ...
  if (!condition) {
    if (state.children) {
      ...
      return changed ? _extends({}, state, { children: res, index: changedIndex }) : state;
    }
    return state;
  }
  var ind = void 0;
  ...
  switch (_Actions.ActionMap[action.type]) {
    ...
    case ActionConst.POP_AND_REPLACE:
      {
        ...
        return _extends({}, state, { children: state.children });
      }
    case ActionConst.REFRESH:
      return props.base ? _extends({
        ...
        key: state.key,
        from: null }) : _extends({}, state, props, {
        key: state.key,
        from: null
      });
    ...
    case ActionConst.JUMP:
      {
        ...
        return _extends({}, state, { index: ind });
      }
